    ...
    public partial class index : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    rptIndex.DataSource = _read.Ebooks();
                    rptIndex.DataBind();
                }
                catch (Exception E)
                {
                    lblErro.Text = "<div class='about-content erro text-center'><p>Houve um erro. Detalhes: " + E.Message + "</p></div>";
                }
        ...
        protected void delete_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                delete.DeleteEbook(int.Parse(e.CommandArgument.ToString()));
                Response.Redirect("/Core/CRUD/Ebook/index.aspx");
            }
            catch (Exception ex)
            {
